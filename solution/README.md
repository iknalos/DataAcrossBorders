...
| **Entity search (LLM labels)** | Ingests the `LLM_output` `GenericCategory` + `FindingTags` into a `finding_tags` table. Search/discovery gain a **status-aware `finding` filter**: default `present` so a report that RULES OUT a finding ("no hydrocephalus") is not returned — the negation problem plain keyword search cannot solve. Also `category` faceting. |
